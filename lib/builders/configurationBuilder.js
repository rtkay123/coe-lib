...
async function configurationBuilder(manager, configurationConfig) {
    console.log('instantiating [config] database');
    ...
    console.log('waiting for [config] database to be ready');
    try {
        const dbReady = await (0, readyCheck_1.isDatabaseReady)(manager._configuration);
        console.log(`[config] db ok: ${dbReady}`);
        ...
        console.log(`[config] db err: ${dbManager_1.readyChecks.PseudonymsDB}`);
    }
