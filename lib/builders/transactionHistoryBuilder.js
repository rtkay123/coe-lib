...
async function transactionHistoryBuilder(manager, transactionHistoryConfig, redis) {
    console.log('instantiating [transactionHistory] database');
    ...
    console.log('waiting for [transactionHistory] database to be ready');
    try {
        const dbReady = await (0, readyCheck_1.isDatabaseReady)(manager._transactionHistory);
        console.log(`[transactionHistory] db ok: ${dbReady}`);
        ...
        console.log(`[transactionHistory] db err: ${dbManager_1.readyChecks.TransactionHistoryDB}`);
    }
